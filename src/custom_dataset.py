...
class CropData(Dataset):
    ...
    def __init__(self, src_dir, usage, dataset_name, csv_path, filtered_images,apply_normalization=False, 
        ...
        img_fnames = [Path(dirpath) / f
              for (dirpath, dirnames, filenames) in os.walk(Path(src_dir+"images/") / self.dataset_name)
              for f in filenames 
              if f.endswith(".tif") 
                      # and ("merged" in f) 
              and f in filtered_images
        ...
        lbl_fnames = [Path(dirpath) / f 
                      for (dirpath, dirnames, filenames) in os.walk(Path(src_dir+"masks/") / self.dataset_name) 
                      for f in filenames 
                      if f.endswith(".tif") 
                      # and ("mask" in f)  
                      and f in filtered_images
        ...
        if self.usage in ["train", "validation"]:
            ...
            for img_fname, lbl_fname in tqdm.tqdm(zip(img_fnames, lbl_fnames),
                ...
                img_id = img_fname
                # '_'.join(img_fname.stem.split('_')[1:3])
