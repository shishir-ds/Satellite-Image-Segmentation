...
class ModelCompiler:
    ...
    def __init__(self, model, working_dir, out_dir, num_classes, inch, class_mapping, gpu_devices=[0],
                 model_init_type="kaiming", params_init=None, freeze_params=None,checkpoint_name=None):
        r"""
        Train the model.
    ...
    def fit(self, trainDataset, valDataset, epochs,checkpoint_name, optimizer_name, lr_init, 
        ...
        path=Path(working_dir) / out_dir / model_dir / checkpoint_name/"chkpt"
        # / out_dir / model_dir / checkpoint_name
        print(path)
        self.checkpoint_dirpath = path

        print(self.checkpoint_dirpath)
        ...
        log_file_path = "/shishir/multi-temporal-crop-classification-baseline/output6/DataParallel_ep1000/training.log"
        
        # Create an empty file (if it doesn't exist) or clear existing content
        ...
        for t in iterable:
            # logging.info("Epoch [{}/{}]".format(t + 1, epochs))
            write_to_log("Epoch [{}/{}]".format(t + 1, epochs))
    
            start_epoch = datetime.now()
            write_to_log("Training will start: {}".format(start_epoch))


            # print("Epoch [{}/{}]".format(t + 1, epochs))

            start_epoch = datetime.now()
            # print("training will start",start_epoch)
            
            train_one_epoch(trainDataset, self.model, criterion, optimizer, 
                            scheduler, device=self.device, 
                            train_loss=train_loss,log_file=log_file_path)
            write_to_log("Training completed")

            # print("Training************************************")
            
            validate_one_epoch(valDataset, self.model, criterion, device=self.device, 
                               val_loss=val_loss,log_file=log_file_path)
            
            write_to_log("Validation complete")
            ...
            if (t + 1) % checkpoint_interval == 0:
                torch.save({"epoch": t + 1,
                           ...
                           os.path.join(self.checkpoint_dirpath, f"{t + 1}_checkpoint.pth"))
        ...
        if save_object == "params":
            if len(self.gpu_devices) > 1:
                ...
                torch.save(self.model.state_dict(),
                           os.path.join(self.checkpoint_dirpath, "{}_final_state.pth".format(self.model_name)))
            print("Save location:",self.checkpoint_dirpath)
