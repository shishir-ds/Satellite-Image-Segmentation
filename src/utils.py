...
def show_random_patches(dataset, sample_num, rgb_bands=(3, 2, 1)):
    ...
    for i, sample_index in enumerate(sample_indices):
        ...
        rgb_data = static_dataset[sample_index][0]
        img_data = rgb_data.view(3, 6, 224, 224)
        ...
        # Here, we select the Red, Green, and Blue bands respectively
            ...
            rgb_image = torch.stack((red_band, green_band, blue_band), dim=0).permute(1, 2, 0) * 1.0
            rgb_image = rgb_image.to(torch.float16)
            # Normalize each channel separately
            normalized_image = (rgb_image / rgb_image.max()) * 255
            normalized_image = normalized_image.to(torch.uint8)
        ...
        #     rgb_data = rgb_data.int()
            ...
            plt.show()
    del static_dataset
    return (normalized_image_np,label_data,rgb_data)
